...
#define EVENT_BEGIN(BINDMAP, T, EVENT, RET) \
...
// RET is a return statement
#define ENTRY_BEGIN(BINDMAP, ENTRY, T, EVENT, RET) \
...
#define ENTRY_EXECUTE(RETVALS) \
    ...
    if (_LuaParams < 2) \
    { \
        ELUNA_LOG_ERROR("[Eluna]: Executing event %u for %s, params was %i. Report to devs", (uint32)_LuaEvent, _LuaBindType, _LuaParams); \
    } \
    for (int j = _LuaFuncTop-_LuaStackTop; j > 0; --j) \
    { \
        for (int i = 0; i <= _LuaParams; ++i) \
...
#define ENDCALL() \
    ...
    { \
        ELUNA_LOG_ERROR("[Eluna]: Ending event %u for %s, stack top was %i and was supposed to be >= %i. Report to devs", (uint32)_LuaEvent, _LuaBindType, lua_gettop(L), _LuaStackTop); \
    } \
    if (_LuaReturnValues != LUA_MULTRET && lua_gettop(L) > _LuaStackTop + _LuaFuncCount * _LuaReturnValues) \
    { \
        ELUNA_LOG_ERROR("[Eluna]: Ending event %u for %s, stack top was %i and was supposed to be between %i and %i. Report to devs", (uint32)_LuaEvent, _LuaBindType, lua_gettop(L), _LuaStackTop, _LuaStackTop + _LuaFuncCount * _LuaReturnValues); \
    } \
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, ELUNA_EVENT_ON_LUA_STATE_CLOSE, return);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, ELUNA_EVENT_ON_LUA_STATE_OPEN, return);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, TRIGGER_EVENT_ON_TRIGGER, return false);
    Push(L, pPlayer);
    Push(L, pTrigger->id);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WEATHER_EVENT_ON_CHANGE, return);
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, AUCTION_EVENT_ON_ADD, return);
    Push(L, (ah));
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, AUCTION_EVENT_ON_REMOVE, return);
    Push(L, (ah));
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, AUCTION_EVENT_ON_SUCCESSFUL, return);
    Push(L, (ah));
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, AUCTION_EVENT_ON_EXPIRE, return);
    Push(L, (ah));
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, SERVER_EVENT_ON_PACKET_SEND, return);
...
{
    ENTRY_BEGIN(PacketEventBindings, OpcodesList(packet.GetOpcode()), PacketEvents, PACKET_EVENT_ON_PACKET_SEND, return);
    Push(L, new WorldPacket(packet));
    Push(L, player);
    ENTRY_EXECUTE(2);
    FOR_RETS(i)
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, SERVER_EVENT_ON_PACKET_RECEIVE, return);
    Push(L, new WorldPacket(packet));
    Push(L, player);
    EVENT_EXECUTE(2);
    FOR_RETS(i)
...
{
    ENTRY_BEGIN(PacketEventBindings, OpcodesList(packet.GetOpcode()), PacketEvents, PACKET_EVENT_ON_PACKET_RECEIVE, return);
    Push(L, new WorldPacket(packet));
    Push(L, player);
    ENTRY_EXECUTE(2);
    FOR_RETS(i)
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, ADDON_EVENT_ON_MESSAGE, return false);
    Push(L, sender);
    Push(L, type);
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_OPEN_STATE_CHANGE, return);
    Push(L, open);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_CONFIG_LOAD, return);
    Push(L, reload);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_SHUTDOWN_INIT, return);
    Push(L, code);
    Push(L, mask);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_SHUTDOWN_CANCEL, return);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
void Eluna::OnWorldUpdate(uint32 diff)
{
    ...
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_UPDATE, return);
    Push(L, diff);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_STARTUP, return);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_SHUTDOWN, return);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    ENTRY_BEGIN(ItemEventBindings, item->GetEntry(), ItemEvents, ITEM_EVENT_ON_GOSSIP_SELECT, return);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_GOSSIP_SELECT, return);
    pPlayer->PlayerTalkClass->ClearMenus();
...
{
    ENTRY_BEGIN(ItemEventBindings, pTarget->GetEntry(), ItemEvents, ITEM_EVENT_ON_DUMMY_EFFECT, return false);
    Push(L, pCaster);
    Push(L, spellId);
    Push(L, effIndex);
    Push(L, pTarget);
...
{
    ENTRY_BEGIN(ItemEventBindings, pItem->GetEntry(), ItemEvents, ITEM_EVENT_ON_QUEST_ACCEPT, return false);
    Push(L, pPlayer);
    Push(L, pItem);
    Push(L, pQuest);
    ENTRY_EXECUTE(0);
...
{
    bool result = true;
    ENTRY_BEGIN(ItemEventBindings, pItem->GetEntry(), ItemEvents, ITEM_EVENT_ON_USE, return result);
    Push(L, pPlayer);
    Push(L, pItem);
#ifndef TRINITY
...
{
    bool result = true;
    ENTRY_BEGIN(ItemEventBindings, pItem->GetEntry(), ItemEvents, ITEM_EVENT_ON_GOSSIP_HELLO, return result);
    pPlayer->PlayerTalkClass->ClearMenus();
    Push(L, pPlayer);
    Push(L, pItem);
    ENTRY_EXECUTE(1);
...
{
    ENTRY_BEGIN(ItemEventBindings, pProto->ItemId, ItemEvents, ITEM_EVENT_ON_EXPIRE, return false);
    Push(L, pPlayer);
...
{
    ENTRY_BEGIN(ItemEventBindings, item->GetEntry(), ItemEvents, ITEM_EVENT_ON_REMOVE, return false);
    Push(L, pPlayer);
    Push(L, item);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
bool Eluna::OnCommand(Player* player, const char* text)
{
    ...
    bool result = true;
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_COMMAND, return result);
    Push(L, player);
    Push(L, fullcmd);
    EVENT_EXECUTE(1);
    FOR_RETS(i)
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_LOOT_ITEM, return);
    Push(L, pPlayer);
    Push(L, pItem);
    Push(L, count);
    Push(L, guid);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_LOOT_MONEY, return);
    Push(L, pPlayer);
    Push(L, amount);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_FIRST_LOGIN, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_REPOP, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_RESURRECT, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_QUEST_ABANDON, return);
    Push(L, pPlayer);
    Push(L, questId);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_EQUIP, return);
    Push(L, pPlayer);
    Push(L, pItem);
    Push(L, bag);
    Push(L, slot);
...
InventoryResult Eluna::OnCanUseItem(const Player* pPlayer, uint32 itemEntry)
{
    ...
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_CAN_USE_ITEM, return result);
    Push(L, pPlayer);
    Push(L, itemEntry);
    EVENT_EXECUTE(1);
    FOR_RETS(i)
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_ENTER_COMBAT, return);
    Push(L, pPlayer);
    Push(L, pEnemy);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_LEAVE_COMBAT, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_KILL_PLAYER, return);
    Push(L, pKiller);
    Push(L, pKilled);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_KILL_CREATURE, return);
    Push(L, pKiller);
    Push(L, pKilled);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_KILLED_BY_CREATURE, return);
    Push(L, pKiller);
    Push(L, pKilled);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_LEVEL_CHANGE, return);
    Push(L, pPlayer);
    Push(L, oldLevel);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_TALENTS_CHANGE, return);
    Push(L, pPlayer);
    Push(L, newPoints);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_TALENTS_RESET, return);
    Push(L, pPlayer);
    Push(L, noCost);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_MONEY_CHANGE, return);
    Push(L, pPlayer);
    Push(L, amount);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_GIVE_XP, return);
    Push(L, pPlayer);
    Push(L, amount);
    Push(L, pVictim);
    EVENT_EXECUTE(1);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_REPUTATION_CHANGE, return);
    Push(L, pPlayer);
    Push(L, factionID);
    Push(L, standing);
    Push(L, incremental);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_DUEL_REQUEST, return);
    Push(L, pTarget);
    Push(L, pChallenger);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_DUEL_START, return);
    Push(L, pStarter);
    Push(L, pChallenger);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_DUEL_END, return);
    Push(L, pWinner);
    Push(L, pLoser);
    Push(L, type);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_EMOTE, return);
    Push(L, pPlayer);
    Push(L, emote);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_TEXT_EMOTE, return);
    Push(L, pPlayer);
    Push(L, textEmote);
    Push(L, emoteNum);
    Push(L, guid);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_SPELL_CAST, return);
    Push(L, pPlayer);
    Push(L, pSpell);
    Push(L, skipCheck);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_LOGIN, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_LOGOUT, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_CHARACTER_CREATE, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_CHARACTER_DELETE, return);
    Push(L, guidlow);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_SAVE, return);
    Push(L, pPlayer);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_BIND_TO_INSTANCE, return);
    Push(L, pPlayer);
    Push(L, difficulty);
    Push(L, mapid);
    Push(L, permanent);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_UPDATE_ZONE, return);
    Push(L, pPlayer);
    Push(L, newZone);
    Push(L, newArea);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_MAP_CHANGE, return);
    Push(L, player);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
bool Eluna::OnChat(Player* pPlayer, uint32 type, uint32 lang, std::string& msg)
{
    ...
    bool result = true;
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_CHAT, return result);
    Push(L, pPlayer);
    Push(L, msg);
    Push(L, type);
    Push(L, lang);
...
bool Eluna::OnChat(Player* pPlayer, uint32 type, uint32 lang, std::string& msg, Group* pGroup)
{
    ...
    bool result = true;
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_GROUP_CHAT, return result);
    Push(L, pPlayer);
    Push(L, msg);
    Push(L, type);
    Push(L, lang);
...
bool Eluna::OnChat(Player* pPlayer, uint32 type, uint32 lang, std::string& msg, Guild* pGuild)
{
    ...
    bool result = true;
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_GUILD_CHAT, return result);
    Push(L, pPlayer);
    Push(L, msg);
    Push(L, type);
    Push(L, lang);
...
bool Eluna::OnChat(Player* pPlayer, uint32 type, uint32 lang, std::string& msg, Channel* pChannel)
{
    ...
    bool result = true;
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_CHANNEL_CHAT, return result);
    Push(L, pPlayer);
    Push(L, msg);
    Push(L, type);
    Push(L, lang);
...
bool Eluna::OnChat(Player* pPlayer, uint32 type, uint32 lang, std::string& msg, Player* pReceiver)
{
    ...
    bool result = true;
    EVENT_BEGIN(PlayerEventBindings, PlayerEvents, PLAYER_EVENT_ON_WHISPER, return result);
    Push(L, pPlayer);
    Push(L, msg);
    Push(L, type);
    Push(L, lang);
...
{
    EVENT_BEGIN(VehicleEventBindings, VehicleEvents, VEHICLE_EVENT_ON_INSTALL, return);
    Push(L, vehicle);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(VehicleEventBindings, VehicleEvents, VEHICLE_EVENT_ON_UNINSTALL, return);
    Push(L, vehicle);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(VehicleEventBindings, VehicleEvents, VEHICLE_EVENT_ON_INSTALL_ACCESSORY, return);
    Push(L, vehicle);
    Push(L, accessory);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(VehicleEventBindings, VehicleEvents, VEHICLE_EVENT_ON_ADD_PASSENGER, return);
    Push(L, vehicle);
    Push(L, passenger);
    Push(L, seatId);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(VehicleEventBindings, VehicleEvents, VEHICLE_EVENT_ON_REMOVE_PASSENGER, return);
    Push(L, vehicle);
    Push(L, passenger);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_ADD_MEMBER, return);
    Push(L, guild);
    Push(L, player);
    Push(L, plRank);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_REMOVE_MEMBER, return);
    Push(L, guild);
    Push(L, player);
    Push(L, isDisbanding);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_MOTD_CHANGE, return);
    Push(L, guild);
    Push(L, newMotd);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_INFO_CHANGE, return);
    Push(L, guild);
    Push(L, newInfo);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_CREATE, return);
    Push(L, guild);
    Push(L, leader);
    Push(L, name);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_DISBAND, return);
    Push(L, guild);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_MONEY_WITHDRAW, return);
    Push(L, guild);
    Push(L, player);
    Push(L, amount);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_MONEY_DEPOSIT, return);
    Push(L, guild);
    Push(L, player);
    Push(L, amount);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_ITEM_MOVE, return);
    Push(L, guild);
    Push(L, player);
    Push(L, pItem);
    Push(L, isSrcBank);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_EVENT, return);
    Push(L, guild);
    Push(L, eventType);
    Push(L, playerGuid1);
    Push(L, playerGuid2);
...
{
    EVENT_BEGIN(GuildEventBindings, HookMgr::GuildEvents, GUILD_EVENT_ON_BANK_EVENT, return);
    Push(L, guild);
    Push(L, eventType);
    Push(L, tabId);
    Push(L, playerGuid);
...
{
    EVENT_BEGIN(GroupEventBindings, GroupEvents, GROUP_EVENT_ON_MEMBER_ADD, return);
    Push(L, group);
    Push(L, guid);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(GroupEventBindings, GroupEvents, GROUP_EVENT_ON_MEMBER_INVITE, return);
    Push(L, group);
    Push(L, guid);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(GroupEventBindings, GroupEvents, GROUP_EVENT_ON_MEMBER_REMOVE, return);
    Push(L, group);
    Push(L, guid);
    Push(L, method);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(GroupEventBindings, GroupEvents, GROUP_EVENT_ON_LEADER_CHANGE, return);
    Push(L, group);
...
{
    EVENT_BEGIN(GroupEventBindings, GroupEvents, GROUP_EVENT_ON_DISBAND, return);
    Push(L, group);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(GroupEventBindings, GroupEvents, GROUP_EVENT_ON_CREATE, return);
    Push(L, group);
    Push(L, leaderGuid);
    Push(L, groupType);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, MAP_EVENT_ON_CREATE, return);
    Push(L, map);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, MAP_EVENT_ON_DESTROY, return);
    Push(L, map);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, MAP_EVENT_ON_PLAYER_ENTER, return);
    Push(L, map);
    Push(L, player);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, MAP_EVENT_ON_PLAYER_LEAVE, return);
    Push(L, map);
    Push(L, player);
    EVENT_EXECUTE(0);
    ENDCALL();
...
void Eluna::OnUpdate(Map* map, uint32 diff)
{
    ...
    // eventMgr->globalProcessor->Update(diff);
    EVENT_BEGIN(ServerEventBindings, ServerEvents, MAP_EVENT_ON_UPDATE, return);
    Push(L, map);
    Push(L, diff);
    EVENT_EXECUTE(0);
    ENDCALL();
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_DELETE_GAMEOBJECT, return);
    Push(L, gameobject);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(ServerEventBindings, ServerEvents, WORLD_EVENT_ON_DELETE_CREATURE, return);
    Push(L, creature);
    EVENT_EXECUTE(0);
    ENDCALL();
}
...
{
    ENTRY_BEGIN(CreatureEventBindings, pTarget->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_DUMMY_EFFECT, return false);
    Push(L, pCaster);
    Push(L, spellId);
    Push(L, effIndex);
    Push(L, pTarget);
...
{
    ENTRY_BEGIN(CreatureEventBindings, pCreature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_GOSSIP_HELLO, return false);
    pPlayer->PlayerTalkClass->ClearMenus();
    Push(L, pPlayer);
    Push(L, pCreature);
    ENTRY_EXECUTE(0);
...
{
    ENTRY_BEGIN(CreatureEventBindings, pCreature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_GOSSIP_SELECT, return false);
    pPlayer->PlayerTalkClass->ClearMenus();
    Push(L, pPlayer);
    Push(L, pCreature);
    Push(L, sender);
...
{
    ENTRY_BEGIN(CreatureEventBindings, pCreature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_QUEST_ACCEPT, return false);
    Push(L, pPlayer);
    Push(L, pCreature);
    Push(L, pQuest);
    ENTRY_EXECUTE(0);
...
{
    ENTRY_BEGIN(CreatureEventBindings, pCreature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_QUEST_REWARD, return false);
    Push(L, pPlayer);
    Push(L, pCreature);
    Push(L, pQuest);
    Push(L, opt);
...
{
    ENTRY_BEGIN(CreatureEventBindings, pCreature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_DIALOG_STATUS, return 0);
    Push(L, pPlayer);
    Push(L, pCreature);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(CreatureEventBindings, creature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_ADD, return);
    Push(L, creature);
    ENTRY_EXECUTE(0);
    ENDCALL();
}
...
{
    ENTRY_BEGIN(CreatureEventBindings, creature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_REMOVE, return);
    Push(L, creature);
    ENTRY_EXECUTE(0);
    ENDCALL();
}
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, pCreature->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_SUMMONED, return false);
    Push(L, pCreature);
    Push(L, pSummoner);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_AIUPDATE, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, diff);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_ENTER_COMBAT, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, target);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_DAMAGE_TAKEN, return false);
    Eluna::Push(L, me);
...
bool Eluna::JustDied(Creature* me, Unit* killer)
{
    On_Reset(me);
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_DIED, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, killer);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_TARGET_DIED, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, victim);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_JUST_SUMMONED_CREATURE, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, summon);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_SUMMONED_CREATURE_DESPAWN, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, summon);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_REACH_WP, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, type);
    Eluna::Push(L, id);
    ENTRY_EXECUTE(1);
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_PRE_COMBAT, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, target);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
bool Eluna::EnterEvadeMode(Creature* me)
{
    On_Reset(me);
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_LEAVE_COMBAT, return false);
    Eluna::Push(L, me);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
    {
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_ATTACKED_AT, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, attacker);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
bool Eluna::JustRespawned(Creature* me)
{
    On_Reset(me);
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_SPAWN, return false);
    Eluna::Push(L, me);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
    {
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_REACH_HOME, return false);
    Eluna::Push(L, me);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
    {
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_RECEIVE_EMOTE, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, player);
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_CORPSE_REMOVED, return false);
    Eluna::Push(L, me);
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_MOVE_IN_LOS, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, who);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_RESET, return);
    Eluna::Push(L, me);
    ENTRY_EXECUTE(0);
    ENDCALL();
}
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_HIT_BY_SPELL, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, caster);
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_SPELL_HIT_TARGET, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, target);
    Eluna::Push(L, spell->Id); // Pass spell object?
    ENTRY_EXECUTE(1);
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_SUMMONED_CREATURE_DIED, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, summon);
    Eluna::Push(L, killer);
    ENTRY_EXECUTE(1);
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_OWNER_ATTACKED_AT, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, attacker);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    bool result = false;
    ENTRY_BEGIN(CreatureEventBindings, me->GetEntry(), CreatureEvents, CREATURE_EVENT_ON_OWNER_ATTACKED, return false);
    Eluna::Push(L, me);
    Eluna::Push(L, target);
    ENTRY_EXECUTE(1);
    FOR_RETS(i)
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pTarget->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_DUMMY_EFFECT, return false);
    Push(L, pCaster);
    Push(L, spellId);
    Push(L, effIndex);
    Push(L, pTarget);
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_GOSSIP_HELLO, return false);
    pPlayer->PlayerTalkClass->ClearMenus();
    Push(L, pPlayer);
    Push(L, pGameObject);
    ENTRY_EXECUTE(0);
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_GOSSIP_SELECT, return false);
    pPlayer->PlayerTalkClass->ClearMenus();
    Push(L, pPlayer);
    Push(L, pGameObject);
    Push(L, sender);
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_QUEST_ACCEPT, return false);
    Push(L, pPlayer);
    Push(L, pGameObject);
    Push(L, pQuest);
    ENTRY_EXECUTE(0);
...
void Eluna::UpdateAI(GameObject* pGameObject, uint32 diff)
{
    ...
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_AIUPDATE, return);
    Push(L, pGameObject);
    Push(L, diff);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_QUEST_REWARD, return false);
    Push(L, pPlayer);
    Push(L, pGameObject);
    Push(L, pQuest);
    Push(L, opt);
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_DIALOG_STATUS, return 0);
    Push(L, pPlayer);
    Push(L, pGameObject);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_DESTROYED, return);
    Push(L, pGameObject);
    Push(L, pPlayer);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_DAMAGED, return);
    Push(L, pGameObject);
    Push(L, pPlayer);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_LOOT_STATE_CHANGE, return);
    Push(L, pGameObject);
    Push(L, state);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(GameObjectEventBindings, pGameObject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_GO_STATE_CHANGED, return);
    Push(L, pGameObject);
    Push(L, state);
    ENTRY_EXECUTE(0);
    ENDCALL();
...
{
    ENTRY_BEGIN(GameObjectEventBindings, gameobject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_SPAWN, return);
    Push(L, gameobject);
    ENTRY_EXECUTE(0);
    ENDCALL();
}
...
{
    ENTRY_BEGIN(GameObjectEventBindings, gameobject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_ADD, return);
    Push(L, gameobject);
    ENTRY_EXECUTE(0);
    ENDCALL();
}
...
{
    ENTRY_BEGIN(GameObjectEventBindings, gameobject->GetEntry(), GameObjectEvents, GAMEOBJECT_EVENT_ON_REMOVE, return);
    Push(L, gameobject);
    ENTRY_EXECUTE(0);
    ENDCALL();
}
...
{
    EVENT_BEGIN(BGEventBindings, BGEvents, BG_EVENT_ON_START, return);
    Push(L, bg);
    Push(L, bgId);
    Push(L, instanceId);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(BGEventBindings, BGEvents, BG_EVENT_ON_END, return);
    Push(L, bg);
    Push(L, bgId);
    Push(L, instanceId);
    Push(L, winner);
...
{
    EVENT_BEGIN(BGEventBindings, BGEvents, BG_EVENT_ON_CREATE, return);
    Push(L, bg);
    Push(L, bgId);
    Push(L, instanceId);
    EVENT_EXECUTE(0);
...
{
    EVENT_BEGIN(BGEventBindings, BGEvents, BG_EVENT_ON_PRE_DESTROY, return);
    Push(L, bg);
    Push(L, bgId);
    Push(L, instanceId);
    EVENT_EXECUTE(0);
