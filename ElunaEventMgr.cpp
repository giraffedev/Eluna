...
LuaEvent::~LuaEvent()
{
    luaL_unref(events->mgr->E->L, LUA_REGISTRYINDEX, funcRef); // Free lua function ref
}

void LuaEvent::Execute()
{
    // In multithread get map from object and the map's lua state
    lua_rawgeti(events->mgr->E->L, LUA_REGISTRYINDEX, funcRef);
    Eluna::Push(events->mgr->E->L, funcRef);
    Eluna::Push(events->mgr->E->L, delay);
    Eluna::Push(events->mgr->E->L, calls);
    if (calls) // Must be before calling
        --calls;
    Eluna::Push(events->mgr->E->L, events->obj);
    events->mgr->E->ExecuteCall(4, 0);
}

ElunaEventProcessor::ElunaEventProcessor(EventMgr* _mgr, WorldObject* _obj) : m_time(0), obj(_obj), mgr(_mgr)
{
    if (obj)
    {
        EventMgr::WriteGuard lock(mgr->GetLock());
        mgr->processors.insert(this);
    }
}
...
EventMgr::EventMgr(Eluna* _E) : globalProcessor(new ElunaEventProcessor(this, NULL)), E(_E)
{
}
