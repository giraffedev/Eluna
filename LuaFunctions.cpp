...
{
    // Hooks
    { ENV_GLOBAL, "RegisterGlobalEvent", &LuaGlobalFunctions::RegisterGlobalEvent },                        // RegisterGlobalEvent(event, function)
    ...
    { ENV_MAP, "RegisterMapEvent", &LuaGlobalFunctions::RegisterMapEvent },                                 // RegisterMapEvent(event, function)
    ...
    { ENV_MAP, "RegisterPlayerEvent", &LuaGlobalFunctions::RegisterPlayerEvent },                           // RegisterPlayerEvent(event, function)
    ...
    { ENV_MAP, "SendStateMsg", &LuaGlobalFunctions::map_SendStateMsg },                                     // SendStateMsg([target_map, target_instanceid, value1, value2...]) - Sends an async message to given map lua state if exists (triggers ELUNA_EVENT_ON_STATE_MSG). Sends to world state if map is nil (or -1).
...
ElunaMethods<Unit> UnitMethods[] =
{
    ...
    // { ENV_GLOBAL, "GetVehicle", &LuaUnit::GetVehicle},                               // :GetVehicle() - Gets the Vehicle kit of the vehicle the unit is on
#endif
    ...
    // { ENV_GLOBAL, "SetStunned", &LuaUnit::SetStunned},                   // :SetStunned([enable]) - Stuns or removes stun
    // { ENV_GLOBAL, "SetRooted", &LuaUnit::SetRooted},                     // :SetRooted([enable]) - Roots or removes root
    // { ENV_GLOBAL, "SetConfused", &LuaUnit::SetConfused},                 // :SetConfused([enable]) - Sets confused or removes confusion
    // { ENV_GLOBAL, "SetFeared", &LuaUnit::SetFeared},                     // :SetFeared([enable]) - Fears or removes fear
...
#endif
    // { ENV_GLOBAL, "SetCanFly", &LuaUnit::SetCanFly},                     // :SetCanFly(apply)
    // { ENV_GLOBAL, "SetVisible", &LuaUnit::SetVisible},                   // :SetVisible(x)
    ...
    // { ENV_GLOBAL, "IsVisible", &LuaUnit::IsVisible},                             // :IsVisible()
    // { ENV_GLOBAL, "IsMoving", &LuaUnit::IsMoving},                               // :IsMoving()
    // { ENV_GLOBAL, "IsFlying", &LuaUnit::IsFlying},                               // :IsFlying()
    ...
    // { ENV_GLOBAL, "IsRooted", &LuaUnit::IsRooted},                               // :IsRooted()
    ...
    // { ENV_GLOBAL, "Kill", &LuaUnit::Kill},                                       // :Kill(target, durabilityLoss) - Unit kills the target. Durabilityloss is true by default
    ...
    // { ENV_GLOBAL, "RestoreDisplayId", &LuaUnit::RestoreDisplayId},               // :RestoreDisplayId()
    // { ENV_GLOBAL, "RestoreFaction", &LuaUnit::RestoreFaction},                   // :RestoreFaction()
    // { ENV_GLOBAL, "RemoveBindSightAuras", &LuaUnit::RemoveBindSightAuras},       // :RemoveBindSightAuras()
    // { ENV_GLOBAL, "RemoveCharmAuras", &LuaUnit::RemoveCharmAuras},               // :RemoveCharmAuras()
    ...
    // { ENV_GLOBAL, "DisableMelee", &LuaUnit::DisableMelee},                       // :DisableMelee([disable]) - if true, enables
    // { ENV_GLOBAL, "SummonGuardian", &LuaUnit::SummonGuardian},                   // :SummonGuardian(entry, x, y, z, o[, duration]) - summons a guardian to location. Scales with summoner, is friendly to him and guards him.
...
ElunaMethods<Player> PlayerMethods[] =
{
    ...
    // { ENV_GLOBAL, "GetRecruiterId", &LuaPlayer::GetRecruiterId},                             // :GetRecruiterId() - Returns player's recruiter's ID
...
#endif
#ifdef CLASSIC
    { ENV_GLOBAL, "GetHonorStoredKills", &LuaPlayer::GetHonorStoredKills},                      // :GetHonorStoredKills(on/off)
    { ENV_GLOBAL, "GetRankPoints", &LuaPlayer::GetRankPoints },                                 // :GetRankPoints()
    { ENV_GLOBAL, "GetHonorLastWeekStandingPos", &LuaPlayer::GetHonorLastWeekStandingPos },     // :GetHonorLastWeekStandingPos()
#endif
...
#endif
#ifdef CLASSIC
    { ENV_GLOBAL, "SetHonorStoredKills", &LuaPlayer::SetHonorStoredKills},                      // :SetHonorStoredKills(kills, [on/off])
    { ENV_GLOBAL, "SetRankPoints", &LuaPlayer::SetRankPoints },                                 // :SetRankPoints(rankPoints)
    { ENV_GLOBAL, "SetHonorLastWeekStandingPos", &LuaPlayer::SetHonorLastWeekStandingPos },     // :SetHonorLastWeekStandingPos(standingPos)
#endif
    ...
    // { ENV_GLOBAL, "SetMovement", &LuaPlayer::SetMovement},                                   // :SetMovement(type) - Sets player's movement type
    ...
    // { ENV_GLOBAL, "IsImmuneToEnvironmentalDamage", &LuaPlayer::IsImmuneToEnvironmentalDamage},   // :IsImmuneToEnvironmentalDamage() - Returns true if the player is immune to enviromental damage
    ...
    // { ENV_GLOBAL, "InRandomLfgDungeon", &LuaPlayer::InRandomLfgDungeon},                         // :InRandomLfgDungeon() - Returns true if the player is in a random LFG dungeon
    // { ENV_GLOBAL, "HasPendingBind", &LuaPlayer::HasPendingBind},                                 // :HasPendingBind() - Returns true if the player has a pending instance bind
#if (!defined(TBC) && !defined(CLASSIC))
    ...
    // { ENV_GLOBAL, "CanFlyInZone", &LuaPlayer::CanFlyInZone},                                     // :CanFlyInZone(mapid, zone) - Returns true if the player can fly in the area
    // { ENV_GLOBAL, "IsNeverVisible", &LuaPlayer::IsNeverVisible},                                 // :IsNeverVisible() - Returns true if the player is never visible
    ...
    // { ENV_GLOBAL, "IsUsingLfg", &LuaPlayer::IsUsingLfg},                                         // :IsUsingLfg() - Returns true if the player is using LFG
    ...
    // { ENV_GLOBAL, "HasReceivedQuestReward", &LuaPlayer::HasReceivedQuestReward},                 // :HasReceivedQuestReward(entry) - Returns true if the player has recieved the quest's reward
#if (!defined(TBC) && !defined(CLASSIC))
...
#endif
    // { ENV_GLOBAL, "IsOutdoorPvPActive", &LuaPlayer::IsOutdoorPvPActive},                         // :IsOutdoorPvPActive() - Returns true if the player is outdoor pvp active
    // { ENV_GLOBAL, "IsARecruiter", &LuaPlayer::IsARecruiter},                                     // :IsARecruiter() - Returns true if the player is a recruiter
    ...
    // { ENV_GLOBAL, "RemovePet", &LuaPlayer::RemovePet},                                           // :RemovePet([mode, returnreagent]) - Removes the player's pet. Mode determines if the pet is saved and how
    // { ENV_GLOBAL, "SummonPet", &LuaPlayer::SummonPet},                                           // :SummonPet(entry, x, y, z, o, petType, despwtime) - Summons a pet for the player
    ...
    // { ENV_GLOBAL, "RemoveActiveQuest", &LuaPlayer::RemoveActiveQuest},                           // :RemoveActiveQuest(entry) - Removes an active quest
    // { ENV_GLOBAL, "RemoveRewardedQuest", &LuaPlayer::RemoveRewardedQuest},                       // :RemoveRewardedQuest(entry) - Removes a rewarded quest
    ...
    // { ENV_GLOBAL, "KilledPlayerCredit", &LuaPlayer::KilledPlayerCredit},                         // :KilledPlayerCredit() - Satisfies a player kill for the player
    // { ENV_GLOBAL, "KillGOCredit", &LuaPlayer::KillGOCredit},                                     // :KillGOCredit(GOEntry[, GUID]) - Credits the player for destroying a GO, guid is optional
    ...
    // { ENV_GLOBAL, "GainSpellComboPoints", &LuaPlayer::GainSpellComboPoints},                     // :GainSpellComboPoints(amount) - Player gains spell combo points
    ...
    // { ENV_GLOBAL, "AddTalent", &LuaPlayer::AddTalent},                                           // :AddTalent(spellid, spec, learning) - Adds a talent spell for the player to given spec
    ...
    // { ENV_GLOBAL, "BindToInstance", &LuaPlayer::BindToInstance},                                 // :BindToInstance() - Binds the player to the current instance
...
#ifdef CLASSIC
    { ENV_GLOBAL, "UpdateHonor", &LuaPlayer::UpdateHonor},                                          // :UpdateHonor() - Updates Player Honor
    { ENV_GLOBAL, "ResetHonor", &LuaPlayer::ResetHonor },                                           // :ResetHonor() - Resets Player Honor
    { ENV_GLOBAL, "ClearHonorInfo", &LuaPlayer::ClearHonorInfo },                                   // :ClearHonorInfo() - Clear Player Honor Info
#endif
...
ElunaMethods<Creature> CreatureMethods[] =
{
    ...
    // { ENV_GLOBAL, "GetWaypointPath", &LuaCreature::GetWaypointPath},                             // :GetWaypointPath() - Returns waypoint path ID
    // { ENV_GLOBAL, "GetCurrentWaypointId", &LuaCreature::GetCurrentWaypointId},                   // :GetCurrentWaypointId() - Returns waypoint ID
    // { ENV_GLOBAL, "GetLootMode", &LuaCreature::GetLootMode},                                     // :GetLootMode() - Returns loot mode
    ...
    // { ENV_GLOBAL, "SetDisableGravity", &LuaCreature::SetDisableGravity},                 // :SetDisableGravity([disable, packetOnly]) - Disables or enables gravity
    ...
    // { ENV_GLOBAL, "SetLootMode", &LuaCreature::SetLootMode},                             // :SetLootMode(lootMode) - Sets the lootmode
    ...
    // { ENV_GLOBAL, "IsTrigger", &LuaCreature::IsTrigger},                                                     // :IsTrigger() - Returns true if the creature is a trigger, false if not
    ...
    // { ENV_GLOBAL, "CanStartAttack", &LuaCreature::CanStartAttack},                                           // :CanStartAttack(unit[, force]) - Returns true if the creature can attack the unit
    ...
    // { ENV_GLOBAL, "IsDamageEnoughForLootingAndReward", &LuaCreature::IsDamageEnoughForLootingAndReward},     // :IsDamageEnoughForLootingAndReward()
    // { ENV_GLOBAL, "HasLootMode", &LuaCreature::HasLootMode},
    ...
    // Other
    // { ENV_GLOBAL, "Despawn", &LuaCreature::Despawn},                             // :Despawn([despawnDelay]) - Creature despawns after given time
    ...
    // { ENV_GLOBAL, "AddLootMode", &LuaCreature::AddLootMode},                     // :AddLootMode(lootMode)
    // { ENV_GLOBAL, "DealDamage", &LuaCreature::DealDamage},                       // :DealDamage(target, amount) - Deals damage to target (if target) : if no target, unit will damage self
    // { ENV_GLOBAL, "SendCreatureTalk", &LuaCreature::SendCreatureTalk},           // :SendCreatureTalk(id, playerGUID) - Sends a chat message to a playerGUID (player) by id. Id can be found in creature_text under the 'group_id' column
    { ENV_GLOBAL, "AttackStart", &LuaCreature::AttackStart },                       // :AttackStart(target) - Creature attacks the specified target
    // { ENV_GLOBAL, "ResetLootMode", &LuaCreature::ResetLootMode},
    // { ENV_GLOBAL, "RemoveLootMode", &LuaCreature::RemoveLootMode},
...
ElunaMethods<GameObject> GameObjectMethods[] =
{
    ...
    // { ENV_GLOBAL, "IsDestructible", &LuaGameObject::IsDestructible},     // :IsDestructible()
...
ElunaMethods<Item> ItemMethods[] =
{
    ...
    // { ENV_GLOBAL, "IsRefundExpired", &LuaItem::IsRefundExpired},             // :IsRefundExpired() - Returns true if the item's refund time has expired
...
ElunaMethods<Quest> QuestMethods[] =
{
    ...
    // { ENV_GLOBAL, "GetMaxLevel", &LuaQuest::GetMaxLevel},                    // :GetMaxLevel() - Returns the quest's max level
